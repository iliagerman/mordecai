...
class TestMemoryServiceSearch:
    ...
    def test_search_memory_returns_facts(self, mock_client_class, mock_config):
        ...
        mock_client.retrieve_memories.return_value = [{"content": {"text": "User likes Python"}}]
        ...
        result = service.search_memory(
            user_id="test-user", query="programming", memory_type="facts"
        )
    ...
    @patch("app.services.memory_service.MemoryClient")
    def test_search_memory_returns_preferences(self, mock_client_class, mock_config):
        ...
        result = service.search_memory(
            user_id="test-user", query="response style", memory_type="preferences"
        )
    ...
    def test_search_memory_all_types(self, mock_client_class, mock_config):
        ...
        result = service.search_memory(user_id="test-user", query="test", memory_type="all")
    ...
    def test_search_memory_no_memory_id(self, mock_client_class, mock_config):
        ...
        result = service.search_memory(user_id="test-user", query="test")

        assert result["facts"] == []
        assert result["preferences"] == []

    @patch("app.services.memory_service.MemoryClient")
    def test_search_memory_handles_exception(self, mock_client_class, mock_config):
    ...
    @patch("app.services.memory_service.MemoryClient")
    def test_search_memory_deduplicates_results(self, mock_client_class, mock_config):
        ...
        result = service.search_memory(user_id="test-user", query="test", memory_type="facts")
    ...
    @patch("app.services.memory_service.MemoryClient")
    def test_search_memory_sorts_newest_first(self, mock_client_class, mock_config):
        """Newest records should be returned first (even if API returns older first)."""
        from app.services.memory_service import MemoryService

        newer = "2026-01-30T12:00:00Z"
        older = "2026-01-29T12:00:00Z"

        mock_client = MagicMock()
        mock_client.retrieve_memories.return_value = [
            {"content": {"text": "Old"}, "eventTimestamp": older},
            {"content": {"text": "New"}, "eventTimestamp": newer},
        ]
        mock_client_class.return_value = mock_client
        ...
        assert result["facts"] == ["New", "Old"]

    @patch("app.services.memory_service.MemoryClient")
    def test_search_memory_dedupe_keeps_newest(self, mock_client_class, mock_config):
        """If the same text appears multiple times, keep the newest timestamp."""
        from app.services.memory_service import MemoryService

        mock_client = MagicMock()
        mock_client.retrieve_memories.return_value = [
            {"content": {"text": "Same"}, "eventTimestamp": "2026-01-01T00:00:00Z"},
            {"content": {"text": "Same"}, "eventTimestamp": "2026-02-01T00:00:00Z"},
            {"content": {"text": "Other"}, "eventTimestamp": "2026-01-15T00:00:00Z"},
        ]
        mock_client_class.return_value = mock_client
        ...
        assert result["facts"] == ["Same", "Other"]
...
class TestMemoryServiceRetrieveContext:
    ...
    @patch("app.services.memory_service.MemoryClient")
    def test_retrieve_memory_context_returns_facts_and_prefs(self, mock_client_class, mock_config):
    ...
    @patch("app.services.memory_service.MemoryClient")
    def test_retrieve_memory_context_sorts_newest_first_and_handles_missing_ts(
        self, mock_client_class, mock_config
    ):
        """Facts and preferences should be newest-first; missing timestamps go last."""
        from app.services.memory_service import MemoryService

        mock_client = MagicMock()
        mock_client.retrieve_memories.side_effect = [
            [
                {"content": {"text": "Old"}, "eventTimestamp": "2026-01-01T00:00:00Z"},
                {"content": {"text": "NoTs"}},
                {"content": {"text": "New"}, "eventTimestamp": "2026-03-01T00:00:00Z"},
            ],
            [
                {"content": {"text": "PrefNew"}, "createdAt": "2026-03-02T00:00:00Z"},
                {"content": {"text": "PrefOld"}, "createdAt": "2026-01-02T00:00:00Z"},
            ],
        ]
        mock_client_class.return_value = mock_client
    ...
    def test_short_term_memories_are_merged_and_overwrite_long_term_by_key(
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            stm_path = Path(tmp) / "me" / "test-user" / "stm.md"
...
class TestMemoryServiceStoreTimestamps:
    """Tests that store_* methods attach event timestamps."""
    ...
    @patch("app.services.memory_service.MemoryClient")
    def test_store_fact_attaches_event_timestamp(self, mock_client_class, mock_config):
        from app.services.memory_service import MemoryService

        mock_client = MagicMock()
        mock_client.retrieve_memories.return_value = []  # _find_similar_records
        mock_client_class.return_value = mock_client
        ...
        assert isinstance(call_kwargs["event_timestamp"], datetime)
        assert call_kwargs["event_timestamp"].tzinfo is not None

    @patch("app.services.memory_service.MemoryClient")
    def test_store_preference_attaches_event_timestamp(self, mock_client_class, mock_config):
        from app.services.memory_service import MemoryService
...
class TestMemoryServiceStoreAgentName:
    ...
    def test_store_agent_name_success(self, mock_client_class, mock_config):
        ...
        result = service.store_agent_name(
            user_id="test-user", name="Mordecai", session_id="test-session"
        )
    ...
    @patch("app.services.memory_service.MemoryClient")
    def test_store_agent_name_creates_memory_if_not_exists(self, mock_client_class, mock_config):
        """Test store_agent_name creates memory if memory_id is None."""
        from app.services.memory_service import MemoryService

        mock_client = MagicMock()
        mock_client.list_memories.return_value = []
        mock_client.create_memory_and_wait.return_value = {"id": "new-memory-id"}
        mock_client_class.return_value = mock_client
        ...
        result = service.store_agent_name(
            user_id="test-user", name="Jarvis", session_id="test-session"
        )
    ...
    @patch("app.services.memory_service.MemoryClient")
    def test_store_agent_name_handles_create_event_failure(self, mock_client_class, mock_config):
    ...
    @patch("app.services.memory_service.MemoryClient")
    def test_store_agent_name_handles_memory_creation_failure(self, mock_client_class, mock_config):
        """Test store_agent_name returns False when memory creation fails."""
        from app.services.memory_service import MemoryService

        mock_client = MagicMock()
        mock_client.list_memories.return_value = []
        mock_client.create_memory_and_wait.side_effect = Exception("Memory creation failed")
        mock_client_class.return_value = mock_client
...
class TestMemoryServiceNameExtraction:
    ...
    @patch("app.services.memory_service.MemoryClient")
    def test_extract_name_from_calls_their_assistant_pattern(self, mock_client_class, mock_config):
    ...
    @patch("app.services.memory_service.MemoryClient")
    def test_extract_name_from_my_name_is_pattern(self, mock_client_class, mock_config):
    ...
    @patch("app.services.memory_service.MemoryClient")
    def test_extract_name_from_call_you_pattern(self, mock_client_class, mock_config):
    ...
    @patch("app.services.memory_service.MemoryClient")
    def test_extract_name_from_call_me_pattern(self, mock_client_class, mock_config):
    ...
    @patch("app.services.memory_service.MemoryClient")
    def test_extract_name_skips_common_words(self, mock_client_class, mock_config):
    ...
    @patch("app.services.memory_service.MemoryClient")
    def test_extract_name_returns_none_for_no_match(self, mock_client_class, mock_config):
    ...
    def test_retrieve_memory_context_extracts_name_from_preferences(
        ...
        result = service.retrieve_memory_context(user_id="test-user", query="what is your name?")
