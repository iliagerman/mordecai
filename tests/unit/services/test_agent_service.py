...
class TestAgentServiceModelProviderProperty:
    """Property-based tests for model provider configuration.
    ...
    **Validates: Requirements 1.2**
    ...
    def test_property_model_provider_matches_config(self, provider: ModelProvider):
        ...
        try:
            is_openai = provider == ModelProvider.OPENAI
            config = AgentConfig(
                model_provider=provider,
                bedrock_model_id="anthropic.claude-3-sonnet-20240229-v1:0",
                openai_model_id="gpt-4",
                openai_api_key="test-key" if is_openai else None,
                telegram_bot_token="test-token",
                session_storage_dir=temp_dir,
                skills_base_dir=temp_dir,
            )
            service = AgentService(config)
            assert service.get_model_provider() == provider
        finally:
    ...
    @settings(max_examples=100)
    @patch("app.services.agent_service.BedrockModel")
    def test_property_bedrock_config_passed_correctly(
        self, mock_bedrock, bedrock_model_id: str, aws_region: str
    ):
        """Property: Bedrock config is always passed correctly to model."""
        temp_dir = tempfile.mkdtemp()
        try:
            config = AgentConfig(
                model_provider=ModelProvider.BEDROCK,
...
class TestAgentServiceNewSessionProperty:
    """Property-based tests for new session functionality.
    ...
    **Validates: Requirements 5.1, 10.1**
    ...
    @patch("app.services.agent_service.BedrockModel")
    @pytest.mark.asyncio
    async def test_property_new_session_always_creates_fresh_agent(
        self, mock_model, mock_agent, user_id: str
    ):
        """Property: new_session always creates a fresh agent."""
        temp_dir = tempfile.mkdtemp()
        try:
            config = AgentConfig(
                model_provider=ModelProvider.BEDROCK,
            ...
            # Create initial agent
            service.get_or_create_agent(user_id)
            initial_call_count = mock_agent.call_count

            # Create new session
            await service.new_session(user_id)

            # Should have created a new agent
            assert mock_agent.call_count == initial_call_count + 1
        finally:
            shutil.rmtree(temp_dir, ignore_errors=True)
    ...
    @patch("app.services.agent_service.BedrockModel")
    @pytest.mark.asyncio
    async def test_property_new_session_clears_session_manager(
        self, mock_model, mock_agent, user_id: str
    ):
        """Property: new_session always clears conversation history."""
        temp_dir = tempfile.mkdtemp()
        try:
            config = AgentConfig(
                model_provider=ModelProvider.BEDROCK,
...
class TestAgentServiceMessageProcessingProperty:
    ...
    **Validates: Requirements 1.3**
    ...
    @patch("app.services.agent_service.Agent")
    @patch("app.services.agent_service.BedrockModel")
    async def test_property_message_produces_response(
        self,
        mock_model,
        mock_agent,
        message: str,
        ...
        """Property: Any valid message produces a non-empty response."""
        temp_dir = tempfile.mkdtemp()
        try:
            # Setup mock
            mock_result = MagicMock()
            ...
            response = await service.process_message("user-1", message)
...
class TestVisionModelSelectionProperty:
    """Property-based tests for vision model selection.
    ...
    **Validates: Requirements 3.1, 3.2, 3.3**
    ...
    @settings(max_examples=100)
    @patch("app.services.agent_service.BedrockModel")
    def test_property_vision_model_selection(
        self,
        mock_bedrock_model,
        vision_model_id: str | None,
        use_vision: bool,
    ):
        """Property: Vision model selection follows configuration rules.

        - If use_vision=True AND vision_model_id is configured -> use vision
        ...
        try:
            ...
            service = AgentService(config)
            service._create_model(use_vision=use_vision)

            # Determine expected model_id
            if use_vision and vision_model_id:
                expected_model_id = vision_model_id
            else:
                expected_model_id = "default-model-id"
...
class TestImageCaptionInclusionProperty:
    """Property-based tests for image caption inclusion.
    ...
    **Validates: Requirements 3.6**
    ...
    @settings(max_examples=100)
    def test_property_caption_inclusion(
        self,
        caption: str | None,
        extension: str,
    ):
        """Property: Caption is included iff it is non-empty."""
        temp_dir = tempfile.mkdtemp()
        try:
            import base64
            ...
            image_path = Path(temp_dir) / f"test{extension}"
            png_data = base64.b64decode(
                "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR4"
                "nGNgYGBgAAAABQABh6FO1AAAAABJRU5ErkJggg=="
            )
            ...
            # Caption block should be present iff caption is truthy
            if caption:
                assert len(content) == 2
                assert content[1]["type"] == "text"
                assert content[1]["text"] == caption
    ...
    @settings(max_examples=100)
    def test_property_media_type_matches_extension(self, extension: str):
        """Property: Media type always matches file extension."""
        temp_dir = tempfile.mkdtemp()
        try:
            config = AgentConfig(
                model_provider=ModelProvider.BEDROCK,
...
class TestAgentServiceWorkingFolderProperty:
    """Property-based tests for working folder in system prompt.
    ...
    **Validates: Requirements 10.3**
    ...
    def test_property_system_prompt_contains_working_folder(self, user_id: str):
        ...
        try:
            ...
            # Property 15: System prompt SHALL contain working folder path
            assert "Working Folder" in prompt
            assert "Your working folder is:" in prompt
            # Path should contain user ID for isolation
            assert user_id in prompt
        finally:
            shutil.rmtree(temp_dir, ignore_errors=True)
    ...
    def test_property_working_folder_path_is_user_isolated(self, user_id: str):
        ...
        try:
            ...
            working_dir = service._get_user_working_dir(user_id)

            # Working folder path must contain user ID
            assert user_id in str(working_dir)
            # Working folder must be under the configured base
            assert str(working_dir).startswith(temp_dir)
        finally:
            shutil.rmtree(temp_dir, ignore_errors=True)
...
class TestGeminiModelProviderProperty:
    """Property-based tests for Google Gemini model provider.

    **Property 1: Gemini Model Creation Consistency**
    *For any* valid API key string, model creation should succeed.
    **Validates: Requirements 4.1, 4.2, 4.3**
    ...
    @settings(max_examples=100)
    @patch("app.services.agent_service.GeminiModel")
    def test_property_gemini_config_passed_correctly(
        self, mock_gemini, api_key: str, model_id: str
    ):
        """Property: Gemini config is always passed correctly to model."""
        temp_dir = tempfile.mkdtemp()
        try:
            config = AgentConfig(
                model_provider=ModelProvider.GOOGLE,
    ...
    def test_property_model_provider_matches_config_all_providers(self, provider: ModelProvider):
        ...
        try:
            is_openai = provider == ModelProvider.OPENAI
            is_google = provider == ModelProvider.GOOGLE
            config = AgentConfig(
                model_provider=provider,
                bedrock_model_id="anthropic.claude-3-sonnet-20240229-v1:0",
                openai_model_id="gpt-4",
                openai_api_key="test-key" if is_openai else None,
                google_model_id="gemini-2.5-flash",
                google_api_key="test-key" if is_google else None,
                telegram_bot_token="test-token",
                session_storage_dir=temp_dir,
                skills_base_dir=temp_dir,
            )
