...
class TestSkillDocumentationStructure:
    ...
    def test_all_skills_have_frontmatter(self, skill_files):
        ...
        for skill_file in skill_files:
            ...
            assert len(frontmatter["description"]) > 10, f"{skill_file} has too short description"
    ...
    def test_all_skills_have_code_examples(self, skill_files):
        ...
        for skill_file in skill_files:
            ...
            assert len(code_blocks) > 0, f"{skill_file} has no code examples"
...
class TestHimalayaSkillDocumentation:
    ...
    def himalaya_skill_path(self) -> Path:
        ...
        path = SKILLS_DIR / "shared" / "himalaya" / "SKILL.md"
        if not path.exists():
    ...
    def test_himalaya_documents_flag_order(self, himalaya_content):
        ...
        assert (
            "--output json date" in himalaya_content
            or "--output json after" in himalaya_content
            or "--output json not" in himalaya_content
        )
...
class TestSkillDocumentationCompleteness:
    ...
    def test_skills_have_installation_check(self, skill_files):
        ...
        for skill_file in skill_files:
            ...
            has_install_docs = any(
                term in content.lower()
                for term in ["install", "prerequisite", "require", "verify", "check"]
            )
            assert has_install_docs, f"{skill_file} missing installation/verification documentation"
    ...
    def test_skills_have_error_handling_docs(self, skill_files):
        ...
        for skill_file in skill_files:
            ...
            has_error_docs = any(
                term in content.lower()
                for term in ["error", "troubleshoot", "issue", "problem", "fail"]
            )
            # This is a soft check - warn but don't fail
            if not has_error_docs:
                pytest.warns(
                    UserWarning, match=f"{skill_file.name} could benefit from error handling docs"
                )
