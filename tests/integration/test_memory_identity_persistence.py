...
pytestmark = [
    pytest.mark.skipif(
        not _can_load_config(), reason="Config with AWS credentials required for AgentCore tests"
    ),
    pytest.mark.asyncio,
]
...
def memory_service(real_config):
    ...
    try:
        ...
        if "UnrecognizedClientException" in msg or "security token" in msg.lower():
            pytest.skip(
                "Config loaded, but AWS credentials are invalid for AgentCore (UnrecognizedClientException)."
            )
        raise
    ...
    while time.time() - start < timeout:
        ...
        try:
            facts = client.retrieve_memories(
                memory_id=memory_id, namespace=f"/facts/{user_id}", query=query, top_k=5
            )
            if facts:
...
class TestIdentityPersistence:
    ...
    @pytest.mark.slow
    async def test_agent_remembers_name_within_session(self, agent_service, unique_user_id):
        """Agent should remember name within same session."""
        # Tell the agent its name
        response1 = await agent_service.process_message(
            unique_user_id, "Your name is Mordecai. Remember that."
        )
        print(f"Response 1: {response1}")

        # Ask for name in same session
        response2 = await agent_service.process_message(unique_user_id, "What is your name?")
        print(f"Response 2: {response2}")

        # Should mention Mordecai
        assert "mordecai" in response2.lower(), (
            f"Agent should remember name 'Mordecai' within session. Got: {response2}"
        )
    ...
    async def test_agent_remembers_name_across_sessions(
        ...
        # Tell the agent its name
        response1 = await agent_service.process_message(
            test_user_id, "I want to call you Mordecai. That is your name now."
        )
        ...
        facts = await wait_for_facts(
            client, memory_id, test_user_id, query="name Mordecai", timeout=60.0, poll_interval=5.0
        )
        ...
        response2 = await agent_service.process_message(test_user_id, "What is your name?")
        print(f"Response 2 (after new session): {response2}")

        # Should still remember Mordecai from long-term memory
        assert "mordecai" in response2.lower(), (
            f"Agent should remember name 'Mordecai' across sessions. Got: {response2}"
        )

    @pytest.mark.slow
    async def test_agent_asks_for_name_when_unknown(self, agent_service, unique_user_id):
        ...
        assert asks_for_name or not_generic, (
            f"Agent should ask for name or not use generic AI name. Got: {response}"
        )
    ...
    async def test_identity_persists_with_multiple_new_sessions(
        ...
        await agent_service.process_message(
            unique_user_id, "Your name is Ziggy. Please remember this."
        )
        ...
        assert "ziggy" in response.lower(), (
            f"Agent should remember 'Ziggy' after multiple sessions. Got: {response}"
        )
...
class TestMemoryStrategies:
    ...
    @pytest.mark.slow
    async def test_preference_strategy_stores_identity(self, agent_service, unique_user_id):
        """userPreferenceMemoryStrategy should store identity preference."""
        # Express preference for name
        await agent_service.process_message(
            unique_user_id, "I prefer to call you Jarvis. Please use that name."
        )
    ...
    @pytest.mark.slow
    async def test_semantic_strategy_stores_identity_fact(self, agent_service, unique_user_id):
        """semanticMemoryStrategy should store identity as fact."""
        # State fact about identity
        await agent_service.process_message(
            unique_user_id, "Remember this fact: Your name is Atlas."
        )
...
class TestMultiUserIsolation:
    ...
    @pytest.mark.slow
    async def test_different_users_different_identities(self, agent_service):
        ...
        await agent_service.process_message(user_a, "Your name is Alpha.")

        # User B names agent "Beta"
        await agent_service.process_message(user_b, "Your name is Beta.")

        await wait_for_memory(3)

        # Check each user's agent identity
        response_a = await agent_service.process_message(user_a, "What is your name?")
        response_b = await agent_service.process_message(user_b, "What is your name?")

        print(f"User A response: {response_a}")
        print(f"User B response: {response_b}")

        assert "alpha" in response_a.lower(), f"User A's agent should be Alpha. Got: {response_a}"
        assert "beta" in response_b.lower(), f"User B's agent should be Beta. Got: {response_b}"
